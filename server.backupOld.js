/**
 * TODO:
 * - verificare serializzazione (super-json o altro)
 * - libreria per suoni: http://soundjax.com
 */
...
var io = require('socket.io'),
    ...
    url = require("url");
var parseCookie = require('connect').utils.parseCookie;  //legato all'authorization
var common = require("./common"),
	session = require("./sessionManager"),
	cards = require("./cards");
	//mysql = require('mysql').createConnection({ user: 'riskio', password: 'riskio', database: 'riskio' }),
	//MySQLStore = require('connect-mysql')(express);

app.configure(function () {
    app.use(express.cookieParser());
    //app.use(express.session({secret: 'secret', key: 'r1s1k0.sid', store: new MySQLStore({ client: mysql })}));
	app.use(express.session({secret: 'secret', key: 'r1s1k0.sid'}));
});

var sessionManager = new session.SessionManager();

/* Main Page */
app.get("/", common.staticHandler("login.html"));

/* ottenimento della lista dei colori disponibili  */
app.get("/coloriDisponibili", function(req, res){
    common.simpleJSON(res, 200, common.colours);//sessionManager().getColoriDisponibili());
});
app.get("/getMatches", function(req, res){
	var matches = sessionManager.getMatchList().getAvailableMatches();
	var list = [];
	for(var i in matches){
		var match = matches[i];
		list.push({id: match.getId(), name: match.getEngine().getMainUser()});
	}
    common.simpleJSON(res, 200, list);
});

/* Join to Map */
app.get("/joinMap", function(req, res){
    var id = req.session.id;
    if ( 1 == 1 && ( !id || id && !sessionManager.checkSessionId(id) ) ){
        res.writeHead(302, {"Location": "/"});
        res.end();
        return;
    }
    common.staticHandler("map.html")(req, res);
});
...
app.get("/join", function(req, res){
    ...
    if (nick === null || nick.length === 0) {
        common.simpleJSON(res, 401, "Bad nick.");
        return;
    }
	...
	if ( sessionManager.addSession(req.session, res) ){
        common.simpleJSON(res, 200, { url: "/joinMap" });
    }
});

/* Join To Match */
app.get("/joinToMatch", function(req, res){

    var q = qs.parse(url.parse(req.url).query);
    var nick = q.nick;
    var color = q.color;
	var matchId = q.matchId;

    if (nick === null || nick.length === 0) {
        common.simpleJSON(res, 401, "Bad nick.");
        return;
    }
	if ( matchId === null || ( matchId !== null && !sessionManager.getMatchList().isAvailableMatch(matchId) ) ){
		common.simpleJSON(res, 401, "Partita non disponibile");
		return;
	}
    ...
    if ( sessionManager.addSession(req.session, res, matchId) ){
...
//app.listen(8000, "0.0.0.0", function(){
	util.log("Starting on port "+app.address().port);
});
var sio = io.listen(app);
sio.set("log level", 1);

sio.sockets.on('connection', function (socket) {
    //util.log('A socket with sessionID ' + socket.handshake.sessionID +' connected!');
	...
	socket.on("firstConnect", function(){
		...
		if ( session ){
			var engine = getEngine(session.matchId);
			if ( !engine ){ return; }
    ...
    socket.on("disconnect", function(){
		util.log("disconnect");
        if ( session ){
            util.log("utente "+session.nick+" disconnesso!");
        }
        ...
        var engine = getEngine(socket.store.data.matchId);
		if ( !engine ){ return; }
        ...
        setTimeout(function(){
            ...
            if ( mysession !== null && mysession.disconnected === true ){
                ...
                sessionManager.removeSession(sessionId); //devo togliere anche il giocatore dal motore, magari dopo 30 secondi...
        ...
        }, 300000);  //5 minuti per tornare in partita!
        sio.sockets.in(socket.store.data.matchId).emit("joinUser", { users: engine.getSessions() });
	...
	socket.on("addTroup", function(data){
		...
		/*
		var obj = addTroupe(session, data, engine);
		var msg = "Generale "+session.nick+", hai ancora "+obj.troupesToAdd+" armate da posizionare!";
		sio.sockets.in(socket.store.data.matchId).emit("troupAddedOnTurn", {
			sessionId: data.sessionId,
			statoId: data.statoId,
			troupes: obj.num,
			troupesRemaining: obj.troupesToAdd,
			troupesFinished: obj.troupesToAdd == 0,
			msg: msg
		});
		*/
		//TODO: controllo!!!!
...
var getEngine = function(matchId){
	/*
	for(idx in sessionManager.getMatchList().getMatchesList()){
		util.log("match online: "+sessionManager.getMatchList().getMatchesList()[idx].getId());
	}
	util.log("MATCH ATTUALE: "+matchId);
	*/
