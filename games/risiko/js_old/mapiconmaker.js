...
MapIconMaker.createMarkerIcon = function (opts) {
  ...
  iconUrl = baseUrl + "&chs=" + width + "x" + height +
      ...
      strokeColor.replace("#", "");
   */
  //eventuale trasparenza
  if(false) {
    iconUrl = iconUrl + "&chf=a,s,ffffff88";
  }
  /*
  icon.imageMap = [
    width / 2, height,
...
MapIconMaker.createFlatIcon = function (opts) {
  ...
  if(false) {
    iconUrl = iconUrl + "&chf=a,s,ffffff01&ext=.png";
  }
  var icon = new google.maps.MarkerImage(iconUrl);
  return icon;
...
MapIconMaker.createLabeledMarkerIcon = function (opts) {
  ...
  var background = opts.background || "dddddd";
  ...
  */
  var baseUrl = "http://chart.apis.google.com/chart?chst=d_bubble_icon_text_small&";
  var iconUrl = "chld=glyphish_user|bbT|"+label+"|"+background+"|000000";
  /*
  if (addStar) {
